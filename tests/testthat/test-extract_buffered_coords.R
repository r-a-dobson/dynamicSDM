...
test_that("Works if temporal.level = year  with prior", {
  ...
  expect_equal(file.exists(paste0(tempdir(),"/",n,"_",test_bandname,"_7_prior_mean_buffered.csv")),TRUE)})
...
test_that("Works if temporal.level = month  with prior", {
  ...
  expect_equal(file.exists(paste0(tempdir(),"/",nameofsplitfile,"_",test_bandname,"_7_prior_mean_buffered.csv")),TRUE)})
...
test_that("Works if temporal.level = year with post", {
  ...
  year<-unique(as.numeric(sample_occ_abs_data_fortest[1,"year"]))
  n<-year[sample(1:length(year),1)]
  expect_equal(file.exists(paste0(tempdir(),"/",n,"_",test_bandname,"_7_post_mean_buffered.csv")),TRUE)})
...
test_that("Works if temporal.level = day with post", {
  ...
  expect_equal(file.exists(paste0(tempdir(),"/",nameofsplitfile,"_",test_bandname,"_7_post_mean_buffered.csv")),TRUE)})
...
test_that("Works if temporal.level = year with combined", {
  ...
  expect_equal(file.exists(paste0(tempdir(),"/all_records_combined_",test_bandname,"_7_post_mean_buffered.csv")),TRUE)})
...
test_that("Works if categories ", {
  ...
  results<-extract_buffered_coords(occ.data=sample_occ_abs_data_fortest,
                                   ...
                                   save.method="combined",
                                   save.directory=tempdir())
  expect_equal(file.exists(paste0(tempdir(),"/all_records_combined_",test_bandname,"_sum_buffered.csv")),TRUE)})
