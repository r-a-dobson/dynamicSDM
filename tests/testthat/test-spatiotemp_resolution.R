...
test_that("stops if wrong class for spatial.res", {
  expect_error(spatiotemp_resolution(occ.data=sample_explan_data,spatial.res="3 decimal places", temporal.res="year",month.infill =1))})

test_that("stops if temporal.res does not match accepted arguments", {
  expect_error(spatiotemp_resolution(occ.data=sample_explan_data, temporal.res="week",day.infill = 2))})
...
test_that("removes records with low spatial resolution", {
  ...
  expect_equal(nrow(result),nrow(sample_explan_data)-1)})
