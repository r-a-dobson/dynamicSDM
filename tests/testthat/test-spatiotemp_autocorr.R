...
test_that("stops if method not accepted", {
  expect_error(spatiotemp_autocorr(occ.data=sample_explan_data2,varname="eight_sum_prec"   ,temporal.level="decadal"))})

test_that("day method output list object", {
  results<-spatiotemp_autocorr(occ.data=sample_explan_data2,varname="eight_sum_prec",temporal.level="day")
...
test_that("year temporal.level works with multiple variables", {
  ...
  results<-spatiotemp_autocorr(occ.data=sample_explan_data2,varname=varnames,temporal.level="year")
  expect_equal(length(results),length(varnames))})

test_that("month temporal.level works with multiple variables", {
  varnames<-c("eight_sum_prec","year_sum_prec")
  results<-spatiotemp_autocorr(occ.data=sample_explan_data2,varname=varnames,temporal.level="month")
    expect_equal(length(results),length(varnames))})
