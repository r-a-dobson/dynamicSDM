...
dynamic_proj <-
  function(dates,
    ...
    projection.method <-
      match.arg(
        projection.method,
        choices = c("proportional", "binary", "abundance" , "stacked"),
        several.ok = T
      )
    ...
    for (x in 1:length(dates)) {
      ...
      if (!missing(local.directory)) {
        ...
        projection_df <- read.csv(filename) # Read in file
      }

      # Read in projection data from Google Drive folder
      if (!missing(drive.folder)) {
        filename <- googledrive::drive_ls(path = drive.folder)$name # List all
        filename <- filename[grep(date, filename)] # Get file name matching date
        filename <- filename[grep("*.csv", filename)] # Get only .csv file name
      ...
      if (!missing(sdm.mod)) {
        ...
        if (class(sdm.mod) == "list") {
          ...
          if (length(sdm.weight) == 1) {
            SDMpred <-
              matrixStats::rowWeightedMeans(projections_blocks,
          ...
          if (length(sdm.weight) > 1) {
            SDMpred <-
              matrixStats::rowWeightedMeans(projections_blocks, w = sdm.weight,
                                            na.rm = T)
          }
      ...
      if (!missing(sam.mod)) {
        ...
        if (class(sam.mod) == "list") {
          ...
          if (length(sam.weight) > 1) {
            SAMpred <- matrixStats::rowWeightedMeans(projections_blocks,
                                                     w = sam.weight,
                                                     na.rm = T)
          }
        }
      ...
      if (!missing(save.drive.folder)) {
        ...
        if (nrow(save.folderpath) == 0) {
          stop("save.drive.folder does not exist.")
        }
        ...
        if (exists("abundancerast")) {
          raster::writeRaster(abundancerast, filename, overwrite = T)
          googledrive::drive_upload(
            media = filename,
            path = googledrive::as_id(save.folderpath$id),
        ...
        if (exists("proportionalrast")) {
          raster::writeRaster(proportionalrast, filename, overwrite = T)
          googledrive::drive_upload(
            media = filename,
            path = googledrive::as_id(save.folderpath$id),
        ...
        if (exists("stackedrast")) {
          raster::writeRaster(stackedrast, filename, overwrite = T)
          googledrive::drive_upload(
            media = filename,
            path = googledrive::as_id(save.folderpath$id),
